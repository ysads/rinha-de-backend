...
def get_stacks_validas():
    stacks = [
        "Javascript", "Python", "Go", "Java", "Kotlin", "PHP", "C#", "Swift", "R", "Ruby", "C", "C++", "Matlab", "TypeScript", "Scala", "SQL", "HTML", "CSS", "NoSQL", "Rust", "Perl", "C#", "Clojure", "MySQL", "Postgres"]
    tem_stack = random.choice([i != 11 for i in range(1, 15)])
    valor_stacks = None
    if tem_stack:
        valor_stacks = random.choices(
            stacks, k=random.randint(1, len(stacks) + 1))
    return valor_stacks


def get_nickname_valido():
...
def get_name_invalido():
    return ''.join(random.choice(string.ascii_letters + ' ') for i in range(120))


def get_birthdate_invalido():
    return json.dumps(random.choice(["12-12-2000", None, 10, "?!?!?"]))


def get_payload():
    gera_nickname_valido = random.choice([i != 1 for i in range(1, 100)])
    get_nickname = get_nickname_valido if gera_nickname_valido else get_nick_invalido

    gera_name_valido = random.choice([i != 1 for i in range(1, 100)])
    get_name = get_name_valido if gera_name_valido else get_name_invalido

    gera_birthdate_valido = random.choice([i != 1 for i in range(1, 100)])
    get_birthdate = get_birthdate_valido if gera_birthdate_valido else get_birthdate_invalido
    ...
    payload = {"nickname": get_nickname(), "name": get_name(
    ), "birthdate": get_birthdate(), "stack": get_stacks()}
    return json.dumps(payload)


def get_termo_busca():
    t = ''.join(random.choice(string.ascii_letters + ' ')
                for i in range(100))[:random.randint(1, 50)]
    return t.strip() or 'x'
